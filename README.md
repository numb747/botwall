...
- **交叉点分析**：`browser` 与 `signed` 是两条相反的经济路径（边际成本 vs 一次性逆向工时），模型给出了"采集量大到多少才值得逆向"的量化答案——本价格表下约 3.2 亿条/年
- 75 项测试，含 7 项端到端

**未完成**
- **让 `browser` 加载真实体积页面 / 引入真实代理**，使阶梯上半段 ~50× 的边际惩罚如实显形。当前 localhost 上只显出约 8×，交叉点因此偏右
